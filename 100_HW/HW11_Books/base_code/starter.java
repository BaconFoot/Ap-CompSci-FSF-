		...
		*/
		while(true){
			...
			if(choice.equals("file")){
				...
				printsNumber(arr,numWord);
			}
			else if(choice.equals("change")){
				System.out.println("What would you like to set your new file to? ");
				textFile = sc.nextLine();
				arr = fillArray(textFile);
			}
			else if(choice.equals("check")){
				System.out.println("What word would you like to check for dupes?");
				String dupeCheck = sc.nextLine();
				System.out.println(dupeCheck + " is used " + checksArray(arr,dupeCheck) + " times.");
	...
	*/
	public static String mostCommon(String[] arr){
		...
		for(int i = 0; i < arr.length; i++){
			...
			if(dupe > mostDupe){
				mostDupe = dupe;
				mostDupeString = arr[i];
			}
		}
		return mostDupeString;
	}
